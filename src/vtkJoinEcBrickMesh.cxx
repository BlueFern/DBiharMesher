...
int vtkJoinEcBrickMesh::RequestData(vtkInformation *vtkNotUsed(request), vtkInformationVector **inputVector, vtkInformationVector *outputVector)
{
	...
	for (int branchId = 0; branchId < 3; branchId++) // TODO: pull num branches from somewhere, branches or just trunk???
	{
		...
		int startPos = (this->Columns * (this->Rows - 1) + 1) + branchOffset;

		cell1  = startPos;
		cell2 = cellsInQuad * this->CircQuads + 1 + branchOffset;
		...
		{
			// Only do the saddle once - with the parent branch.
			if (quadId >= quadsInBranch - this->CircQuads && branchId != 0)
			{
				break;
			}
			...
			if (fixes == 0)
			{
				...
				cell2 = (cellsInQuad * quadId) + (cellsInQuad * this->CircQuads) + 1 + branchOffset;
				fixes = this->Columns / 2;

				// The bottom half of the saddle ring, requires a different algorithm to calculate the next cell to be connected to.
				if (quadId >= quadsInBranch - this->CircQuads && quadId >= (this->AxialQuads - 1) * this->CircQuads + this->CircQuads / 2)
				{
					cell2 = (cellsInQuad * this->CircQuads) + (cellsInQuad * quadId) + 1 + (quadsInBranch * cellsInQuad);
				}
			}
			else
			{
