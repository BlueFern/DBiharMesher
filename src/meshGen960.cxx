...
int main(int argc, char* argv[]) {
	...
	vesselCentrelineReader->SetFileName("c960Centreline.vtk");
	...
	rescaleUnits->SetScale(960); // Convert mm to µm.
	rescaleUnits->Update();
	...
	// vtkDbiharStatic::WritePolyData(fullMeshJoiner->GetOutput(), (prefix + "quadMeshFullc960.vtp").c_str());
	vtkDbiharStatic::WritePolyData(fullMeshJoiner->GetOutput(), "quadMeshFullc960.vtp");
	...
	// vtkDbiharStatic::WritePolyData(subdivideECMesh->GetOutput(), (prefix + "quadMeshFullECc960.vtp").c_str());
	vtkDbiharStatic::WritePolyData(subdivideECMesh->GetOutput(), "quadMeshFullECc960.vtp");
	...
	// vtkDbiharStatic::WritePolyData(subdivideSMCMesh->GetOutput(), (prefix + "quadMeshFullSMCc960.vtp").c_str());
	vtkDbiharStatic::WritePolyData(subdivideSMCMesh->GetOutput(), "quadMeshFullSMCc960.vtp");
	...
	for (int i = 0; i < endPointIds->GetNumberOfIds(); i++)
	{
		...
		// vtkDbiharStatic::WriteStlData(triangleCapFilter->GetOutput(), (prefix + SSTR("triCap_" << i << "_c960.stl")).c_str());
		vtkDbiharStatic::WriteStlData(triangleCapFilter->GetOutput(), (SSTR("triCap_" << i << "_c960.stl")).c_str());
	}
	...
	// vtkDbiharStatic::WriteStlData(triangleFullMeshFilter->GetOutput(), (prefix + "triMeshFullc960.stl").c_str());
	vtkDbiharStatic::WriteStlData(triangleFullMeshFilter->GetOutput(), "triMeshFullc960.stl");
#endif
