...
int main(int argc, char* argv[]) {
	...
	// Half a ring.
	int numRadialQuads = 4;

	vtkSmartPointer<vtkGenericDataObjectReader> vesselCentrelineReader = vtkSmartPointer<vtkGenericDataObjectReader>::New();
	vesselCentrelineReader->SetFileName("c32Centreline.vtk");
	...
	// vtkDbiharStatic::WritePolyData(centrelinePartitioner->GetOutput(), "c32CentrelinePartitioned.vtp");
	...
	while(true)
	{
		...
		vtkDbiharStatic::WritePolyData(dbiharPatchFilter->GetOutput(), SSTR(spineId) + "_Dbihar_Out.vtp");
		...
		if(inputId > maxInputId)
		{
			...
			// vtkDbiharStatic::PrintDataArray(pointsToMeshDimensions);
	...
	// vtkDbiharStatic::WritePolyData(fullMeshJoiner->GetOutput(), (prefix + "quadMeshFullc32.vtp").c_str());
	vtkDbiharStatic::WritePolyData(fullMeshJoiner->GetOutput(), "quadMeshFullc32.vtp");
	...
	// vtkDbiharStatic::WritePolyData(subdivideECMesh->GetOutput(), (prefix + "quadMeshFullECc32.vtp").c_str());
	vtkDbiharStatic::WritePolyData(subdivideECMesh->GetOutput(), "quadMeshFullECc32.vtp");
	...
	// vtkDbiharStatic::WritePolyData(subdivideSMCMesh->GetOutput(), (prefix + "quadMeshFullSMCc32.vtp").c_str());
	vtkDbiharStatic::WritePolyData(subdivideSMCMesh->GetOutput(), "quadMeshFullSMCc32.vtp");
	...
	for (int i = 0; i < endPointIds->GetNumberOfIds(); i++)
	{
		...
		// vtkDbiharStatic::WriteStlData(triangleCapFilter->GetOutput(), (prefix + SSTR("triCap_" << i << "_c32.stl")).c_str());
		vtkDbiharStatic::WriteStlData(triangleCapFilter->GetOutput(), (SSTR("triCap_" << i << "_c32.stl")).c_str());
	}
	...
	// vtkDbiharStatic::WriteStlData(triangleFullMeshFilter->GetOutput(), (prefix + "triMeshFullc32.stl").c_str());
	vtkDbiharStatic::WriteStlData(triangleFullMeshFilter->GetOutput(), "triMeshFullc32.stl");
#endif
