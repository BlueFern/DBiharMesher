...
int main(int argc, char* argv[]) {
	...
	// Now create smc and ec meshes.

	int numECs = 4;
	int numSMCs = 4;
	vtkSmartPointer<vtkSubdivideMeshBrick> subdivideECMesh = vtkSmartPointer<vtkSubdivideMeshBrick>::New();
	...
	subdivideECMesh->SetCellType(vtkDbiharStatic::EC);
	subdivideECMesh->SetAxialQuads(34);
	subdivideECMesh->SetCircQuads(40);
	subdivideECMesh->Update();

	vtkDbiharStatic::WritePolyData(subdivideECMesh->GetOutput(), "quadMeshFullECc4080.vtp");

	vtkSmartPointer<vtkSubdivideMeshBrick> subdivideSMCMesh = vtkSmartPointer<vtkSubdivideMeshBrick>::New();
