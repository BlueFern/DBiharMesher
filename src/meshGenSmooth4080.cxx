...
int main(int argc, char* argv[])
{
	...
	dbiharPatchSmooth->SetInputData(fullMeshJoiner->GetOutput());
	dbiharPatchSmooth->SetNumRadialQuads(numRadialQuads);
	dbiharPatchSmooth->Update();


	vtkDbiharStatic::ShowPolyData(dbiharPatchSmooth->GetOutput());
	vtkDbiharStatic::WritePolyData(dbiharPatchSmooth->GetOutput(), "quadMeshFullSmoothc4080.vtp");

	int numECs = 4;
	int numSMCs = 4;
	vtkSmartPointer<vtkSubdivideMesh> subdivideECMesh = vtkSmartPointer<vtkSubdivideMesh>::New();
	subdivideECMesh->SetInputData(dbiharPatchSmooth->GetOutput());
	...
	vtkDbiharStatic::WritePolyData(subdivideECMesh->GetOutput(), "quadMeshFullSmoothECc4080.vtp");

	vtkSmartPointer<vtkSubdivideMesh> subdivideSMCMesh = vtkSmartPointer<vtkSubdivideMesh>::New();
	subdivideSMCMesh->SetInputData(dbiharPatchSmooth->GetOutput());
	...
	vtkDbiharStatic::WritePolyData(subdivideSMCMesh->GetOutput(), "quadMeshFullSmoothSMCc4080.vtp");
	...
	for (int i = 0; i < endPointIds->GetNumberOfIds(); i++)
	{
		...
		dbiharPatchSmooth->AddInputData(skipSegmentFilter->GetOutput());
		...
		vtkDbiharStatic::WriteStlData(triangleCapFilter->GetOutput(), (SSTR("triCap_" << i << "Smooth_c4080.stl")).c_str());
	}


	vtkSmartPointer<vtkTriangleFilter> triangleFullMeshFilter = vtkSmartPointer<vtkTriangleFilter>::New();
	triangleFullMeshFilter->AddInputData(dbiharPatchSmooth->GetOutput());

	triangleFullMeshFilter->Update();

	vtkDbiharStatic::WriteStlData(triangleFullMeshFilter->GetOutput(), "triMeshFullSmoothc4080.stl");
